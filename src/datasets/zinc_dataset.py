...
class Selectparp1Transform:
    def __call__(self, data):
        data.y = data.y[..., :1]
        return data

class Selectfa7Transform:
    def __call__(self, data):
        data.y = data.y[..., 1:2]
        return data

class Select5ht1bTransform:
    def __call__(self, data):
        data.y = data.y[..., 2:3]
        return data

class SelectbrafTransform:
    def __call__(self, data):
        data.y = data.y[..., 3:4]
        return data

class Selectjak2Transform:
    def __call__(self, data):
        data.y = data.y[..., 4:5]
        return data
...
class MOSESDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        qed = df["qed"].values
        sa  = df["sa"].values
        target_list = ["parp1","fa7","5ht1b","braf","jak2"]
        score_list   = []
        for target in target_list:
            ds = df[target].values/20
            score_list.append(qed*sa*ds)
        data_list = []
        smiles_kept = []
        ...
        for i, smile in enumerate(tqdm(smiles_list)):
            ...
            y = torch.zeros(size=(1, 5), dtype=torch.float)
            for tidx in range(5):
                y[:,tidx] = score_list[tidx][i]
...
class MosesDataModule(MolecularDataModule):
    ...
    def prepare_data(self):
        target = self.cfg.general.target
        if target == "parp1":
            transform = Selectparp1Transform()
        elif target == "fa7":
            transform = Selectfa7Transform()
        elif target == "5ht1b":
            transform = Select5ht1bTransform()
        elif target == "braf":
            transform = SelectbrafTransform()
        elif target == "jak2":
            transform = Selectjak2Transform()
        else:
            raise ValueError
        ...
        base_path = pathlib.Path(os.path.realpath(__file__)).parents[2]
        root_path = os.path.join(base_path, self.datadir)
        datasets = {'train': MOSESDataset(stage='train', root=root_path, filter_dataset=self.filter_dataset,transform=transform),
                    'val': MOSESDataset(stage='val', root=root_path, filter_dataset=self.filter_dataset,transform=transform),
                    'test': MOSESDataset(stage='test', root=root_path, filter_dataset=self.filter_dataset,transform=transform)}
        super().prepare_data(datasets)
